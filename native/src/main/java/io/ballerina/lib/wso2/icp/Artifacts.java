    ...
    public static Object getMainArtifact(Environment env) {
        ...
        for (Artifact artifact : allArtifacts) {
            if (artifact.type.toString().equals(MAIN)) {
                ...
                Object packageOrg = details.get(PACKAGE_ORG);
                Object packageName = details.get(PACKAGE_NAME);
                Object packageVersion = details.get(PACKAGE_VERSION);

                if (packageOrg == null) {
                    missingFields.add(PACKAGE_ORG);
                }
                if (packageName == null) {
                    missingFields.add(PACKAGE_NAME);
                }
                if (packageVersion == null) {
                    missingFields.add(PACKAGE_VERSION);
                }

                if (!missingFields.isEmpty()) {
                    String missingFieldsStr = String.join(", ", missingFields);
                    return ErrorCreator.createError(StringUtils.fromString(
                            "Main artifact is missing required field(s): " + missingFieldsStr));
                }

                BMap<BString, Object> mainDetail = ValueCreator.createMapValue();
                mainDetail.put(StringUtils.fromString(PACKAGE_ORG),
                        StringUtils.fromString((String) packageOrg));
                mainDetail.put(StringUtils.fromString(PACKAGE_NAME),
                        StringUtils.fromString((String) packageName));
                mainDetail.put(StringUtils.fromString(PACKAGE_VERSION),
                        StringUtils.fromString((String) packageVersion));
